...
CARROT_SETTINGS = {
  ...
  "ComfortBrake": {
    ...
    "help": "플래너가 <b>내가 편안하게 낼 수 있다고 가정하는 감속도</b>입니다. 목표거리 곡선의 "
            ...
            "45mph 기준 목표거리를 약 13m 줄입니다 — 갭 표(거리)는 예전의 잘 달리던 값으로 되돌려 둔 "
            "상태라(2026-09-01, gap4 t_follow 0.88초), 이 값은 곡선의 곡률만 건드린다고 보면 됩니다."
  ...
  "LowSpeedJerk": {
    ...
    "help": "정체 구간에서 <b>가속을 얼마나 급하게 바꿔도 되는지</b>를 정합니다. MPC 의 "
            "<code>a_change_cost</code> 와 <code>J_EGO_COST</code> 에 함께 곱해지는 값이라, "
            "<b>클수록 가속 변화가 비싸져서 부드러워집니다</b>."
            "<br><br>carrot 은 이 값을 <b>차간거리 단계로만</b> 정합니다 (1단 0.5 ~ 7단 1.0). "
            "즉 <b>바짝 붙고 싶다고 고르면 울컥거림도 같이 딸려옵니다</b>. 둘은 별개의 요구인데 "
            "한 축에 묶여 있는 것이 문제입니다."
            "<br><br>실측(09-09 저녁 정체, 000000f8 seg 14~23, 인게이지 9.5분 중 95%가 40km/h "
            "미만, 차간 2단이라 계수 <b>0.567</b>): 명령이 ±0.5 m/s² 를 <b>61회</b> 넘나들어 "
            "<b>9.4초마다 한 번</b>씩 붕→끽이 반복됐고, 기어가는 속도에서 가속은 상한 "
            "<b>+2.00</b> 에 붙고 제동은 <b>-3.16</b> 까지 갔습니다 (제동 p90 -2.05)."
            "<br><br>이 값은 <b>저속에서만 계수의 하한</b>으로 들어갑니다: 10km/h 이하에서 이 값, "
            "40km/h 에서 0 으로 사라집니다. <b>차간거리는 건드리지 않습니다</b> — 붙는 거리는 그대로 "
            "두고 다가가는 방식만 부드럽게 합니다. 올리는 방향으로만 작동하므로 이미 부드러운 "
            "차간 단계(6~7단)에는 아무 영향이 없고, 고속 주행도 그대로입니다."
            "<br><br>0 이면 끕니다(carrot 원래 동작). 100 이면 저속에서 7단과 같은 부드러움입니다.",
    "range": (0, 100, 5), "scale": 0.01, "zero": "끔 (carrot 원래대로)",
  },
  "TeslaBrakeJerk": {
    "label": "제동 급격함 제한 (드르륵 소리)", "type": "int",
    "help": "차에 <b>브레이크를 얼마나 빨리 물려도 되는지</b>(DAS_jerkMin)를 알려주는 값의 하한입니다. "
            "0 이면 지금처럼 항상 최대 권한(4.9, 폴트가 5.0)을 줍니다."
            "<br><br>실측(09-02·09-03, 운전자 페달 제외): openpilot 이 요구하는 저크는 중앙 "
            "<b>0.41</b> m/s³ (p90 1.07) 로 온건한데, 차가 실제로 내는 저크는 중앙 <b>0.97</b> "
            "(p90 3.5) 로 <b>2.4배</b>이고 5% 는 4.9 를 넘겼습니다. 지령보다 훨씬 급하게 물렸다 "
            "풀었다 하는 것이 속도 있는 상태에서 들리는 '드르륵' 입니다."
            "<br><br>이 값을 켜면 <b>요구한 만큼만 권한을 줍니다</b>: "
            "<code>허용 = clip(max(이값, 2.0 × 지령저크), 이값, 4.9)</code>. 평상시(0.41)엔 이 값 "
            "그대로, 정말 급할 땐 지금과 같은 4.9까지 즉시 열립니다. 권한은 <b>수요가 커지면 즉시 "
            "열리고 천천히 닫히므로</b>, 조여도 실제 제동 요구를 놓치지 않습니다."
            "<br><br>CarrotPilot 이 현대차에서 같은 문제를 1.2 하한으로 푸는데, 그쪽 여는 조건"
            "(지령 감속 -2.8 초과)은 이 차에선 <b>0.0%</b> 라 그대로 쓰면 상시 1.2 로 묶여버립니다. "
            "그래서 여는 기준을 지령 accel 이 아니라 <b>지령 저크</b>로 바꿨습니다."
            "<br><br><b>제동거리 비용 (실측)</b> — 09-04 로그의 실제 제동 17건(급제동 9건, 시작속도 "
            "중앙 73km/h·최고 121km/h)을 각 설정에 통과시켜 잰 <b>추가 제동거리</b>, 그리고 "
            "100km/h 순항 중 지령이 즉시 최대제동(-4.2)으로 뛰는 최악의 경우:"
            "<br><table style='border-collapse:collapse'>"
            "<tr><th style='padding:2px 8px'>설정</th><th style='padding:2px 8px'>실주행 중앙</th>"
            "<th style='padding:2px 8px'>실주행 최대</th><th style='padding:2px 8px'>최악(100km/h)</th></tr>"
            "<tr><td>0.8</td><td>+1.4m</td><td>+3.5m</td><td>+10.2m</td></tr>"
            "<tr><td>1.0</td><td>+1.3m</td><td>+3.3m</td><td>+9.5m</td></tr>"
            "<tr><td>1.2</td><td>+1.2m</td><td>+3.1m</td><td>+9.0m</td></tr>"
            "<tr><td>1.5</td><td>+1.1m</td><td>+2.9m</td><td>+8.2m</td></tr>"
            "<tr><td>2.0</td><td>+1.0m</td><td>+2.5m</td><td>+6.8m</td></tr>"
            "<tr><td>2.5</td><td>+0.6m</td><td>+1.6m</td><td>+5.6m</td></tr>"
            "<tr><td>3.0</td><td>+0.4m</td><td>+1.1m</td><td>+4.4m</td></tr>"
            "<tr><td>4.0</td><td>+0.2m</td><td>+0.4m</td><td>+2.1m</td></tr>"
            "</table>"
            "<b>실주행에서는 가장 강하게 조여도 추가 제동거리가 3.5m 를 넘지 않습니다.</b> "
            "설정 간 차이가 작은 이유는, 급수요가 들어오면 상한이 0.3초 안에 어차피 최대까지 "
            "열리기 때문입니다 — 하한은 평상시에만 작용합니다."
            "<br><br><b>이 값은 급제동 소음에는 거의 듣지 않습니다.</b> 하한 0.8 로 주행한 실측에서 "
            "평상시 제동은 하한에 정확히 앉았지만(중앙 -0.80), 급제동에서는 수요를 따라 상한이 "
            "p10 -4.37 / 최악 -4.79 까지 열렸습니다. 급제동 때 나는 소리를 줄이려면 아래의 "
            "<b>제동 저크 천장</b>을 쓰세요."
  ...
  "TeslaBrakeJerkMax": {
    ...
    "help": "위 하한이 <b>평상시</b> 제동을 다스린다면, 이 값은 <b>급제동</b>을 다스립니다. "
            "수요가 커져도 이 이상으로는 상한을 열지 않습니다. 0 이면 지금처럼 4.9(폴트 직전)까지 "
            "열립니다."
            "<br><br>왜 필요한가: 하한 0.8 로 주행한 실측에서 평상시 제동은 하한에 정확히 "
            "앉았지만(중앙 -0.80, 2454 프레임), <b>급제동 189 프레임에서는 p10 -4.37 / 최악 -4.79</b> "
            "까지 열렸고 14.8% 가 -4.0 을 넘겼습니다. 급정거 때 남는 소리는 여기서 나옵니다."
            "<br><br><b>⚠ 하한과 달리 이 값은 진짜 급제동을 제한합니다.</b> 실측 제동 14건(급제동 "
            "10건, 시작속도 중앙 67km/h)과 100km/h 계단 최악의 추가 제동거리:"
            "<br><table style='border-collapse:collapse'>"
            "<tr><th style='padding:2px 8px'>천장</th><th style='padding:2px 8px'>실주행 중앙</th>"
            "<th style='padding:2px 8px'>실주행 최대</th><th style='padding:2px 8px'>최악(100km/h)</th></tr>"
            "<tr><td>4.0</td><td>+0.3m</td><td>+1.8m</td><td>+8.6m</td></tr>"
            "<tr><td>3.5</td><td>+0.5m</td><td>+3.3m</td><td>+14.3m</td></tr>"
            "<tr><td>3.0</td><td>+0.8m</td><td>+5.5m</td><td>+20.6m</td></tr>"
            "<tr><td>2.5</td><td>+1.3m</td><td>+9.0m</td><td>+27.3m</td></tr>"
            "<tr><td>2.0</td><td>+2.0m</td><td>+14.4m</td><td>+34.2m</td></tr>"
            "</table>"
  ...
  "TeslaICLeadMaxM": {
    ...
    "help": "openpilot 이 100m 쯤의 앞차를 인지해도 계기판이 안 그리는 경우가 있습니다. 이 값을 "
            "켜면 그보다 먼 앞차를 <b>계기판에만</b> 이 거리로 당겨서 보냅니다. 0 이면 사실 그대로."
            "<br><br>필드 한계는 아닙니다 — <code>DAS_objVehDx</code> 는 127.5m 까지 담고, "
            "<b>공장도 최대 124.5m 까지 물체를 보냅니다</b>(RelevantForControl 켜진 채로도 동일). "
            "실측: 순정 캡처에서 객체의 1.0%, 우리 주행에서 8.4% 가 100m 초과였습니다. 즉 못 그리는 "
            "이유는 계기판 <b>내부 표시 규칙</b>이고, 그건 버스에서 보이지 않습니다. 값을 바꿔가며 "
            "어디서부터 그려지는지 찾는 용도입니다."
            "<br><br><b>⚠ 이건 운전자에게 사실과 다른 거리를 보여줍니다.</b> 항상 실제보다 "
            "<b>가깝게만</b> 표시되고(멀게 표시되는 일은 없음) 표시 전용 메시지라 제어에는 전혀 "
            "영향이 없지만, 그 점을 알고 쓰셔야 합니다. 설정한 값보다 가까운 앞차는 그대로입니다."
            "<br><br><b>고정이 아니라 압축입니다.</b> 임계값 이하는 그대로 두고, 그보다 먼 거리만 "
            "1/4 로 눌러 보냅니다(80 설정 시 80→80, 100→85, 120→90). 처음엔 하드 클램프였는데 "
            "실주행에서 주입된 물체 67개가 <b>전부 정확히 80.0m</b> 로 나가, 앞차가 80m 에 얼어붙어 "
            "있다가 그 안으로 들어와야 움직이기 시작했습니다. 압축은 다가오는 움직임을 살립니다."
            "<br><br>80 → 75 → 70 순으로 내려보며 그려지기 시작하는 지점을 찾아보세요.",
    "options": [(0, "사실 그대로 (기본)"), (90, "90m"), (80, "80m"), (75, "75m"), (70, "70m"), (60, "60m")],
  },
  "RadarLeadHoldCm": {
    "label": "앞차 놓침 방지 (레이더 유지)", "type": "int",
    "help": "비전이 앞차를 잠깐 놓쳤을 때 <b>레이더가 계속 보고 있는 그 차를 그대로 유지</b>합니다. "
            "설정한 거리 안쪽에서만 동작하며 0 이면 사용하지 않습니다."
            "<br><br>실측(09-01·09-02 주행): 레이더↔비전 전환의 <b>90%가 비전은 같은 차를 계속 "
            "보고 있는데 거리만 5.7~11.9m 튀었습니다</b> — 물체가 바뀐 게 아니라 레이더 연결이 "
            "끊긴 것입니다. 그 순간 플래너에는 <b>앞차가 갑자기 가까워진 것</b>으로 보여 급제동이 "
            "나갑니다. 앞차를 놓친 거리는 중앙값으로 각각 44m·97m 였습니다."
            "<br><br>이미 확정됐던 차만 붙잡고, <b>레이더가 지금도 측정 중</b>이며 트랙이 튀지 "
            "않았고 경로에서 벗어나지 않은 동안에만 유지합니다(60m 이내 0.8m, 그 밖 1.2m). "
            "사라진 물체를 상상해서 브레이크를 밟는 구조가 아닙니다."
            "<br><br>20m 는 근거리 전용이던 원래 설정이라 위 소실 구간에 닿지 않습니다.",
    "options": [(0, "사용 안 함"), (2000, "20m (근거리만)"), (6000, "60m"),
                (10000, "100m (권장)"), (15000, "150m")],
  },
  "RadarLeadHoldMs": {
    "label": "└ 유지 시간", "type": "int",
    "help": "위 유지를 최대 몇 ms 까지 이어갈지입니다. 이 예산을 다 쓰면 놓아주고, 비전이 다시 "
            "같은 차를 잡으면 예산이 복구됩니다. 길수록 깜빡임을 잘 덮지만 실제로 빠져나간 앞차를 "
            "늦게 놓아줍니다.",
    "options": [(500, "0.5초"), (1000, "1.0초 (기본)"), (1500, "1.5초"), (2000, "2.0초")],
  },
  "TeslaCutInLead": {
...
SETTINGS = {
  ...
  "LatSmoothSec": {
    "label": "조향 떨림 완화 (저속)", "type": "int",
    "help": "모델이 내놓는 목표 곡률에 <b>저역통과 필터</b>를 겁니다. 값은 시정수(초)입니다."
            "<br><br>모델은 곡률이 아니라 <code>곡률×속도²</code> 성격의 값을 내놓고 modeld 가 "
            "속도²로 나눠 곡률을 만듭니다. 그래서 <b>같은 모델 노이즈가 저속에서 1/속도² 로 "
            "증폭</b>됩니다 — 실측으로 3~7km/h 구간의 곡률 지터가 고속의 13~17배였고, 앞차를 "
            "보고 멈추는 동안 지령 각속도가 최대 1341°/s(하드웨어 한계 250°/s)까지 요구되어 "
            "핸들이 약 2Hz로 떨렸습니다. 진폭은 1~3m/s 구간이 가장 커서 4.8° 였습니다."
            "<br><br>필터라서 <b>정상상태 곡률은 그대로</b>입니다 — 저속 회전 능력을 깎지 않고 "
            "지터만 걷어냅니다. 모델이 경로를 확신 못 할 때(1초 앞 횡위치 표준편차 0.15~0.25m)는 "
            "자동으로 최대 2배까지 더 걸리며, 총합은 0.60초로 제한됩니다. 필터 지연은 모델의 "
            "예측 시점을 그만큼 앞당겨 보상합니다."
            "<br><br>업스트림은 같은 장치를 종방향에는 0.3초로 켜두고 횡방향만 0으로 꺼둔 "
            "상태이고, CarrotPilot 은 0.13초로 켭니다. 0 이면 기존 동작과 완전히 동일합니다.",
    "options": [(0, "사용 안 함 (기본)"), (8, "약하게 0.08초"), (13, "carrot 기본 0.13초"),
                (20, "강하게 0.20초"), (30, "매우 강하게 0.30초")],
  },
