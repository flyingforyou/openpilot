...
MAP_SETTINGS = {
  ...
  "TeslaMapAutoSpeedCurve": {
    ...
    "help": "그 지점을 실제로 달리는 플릿 속도(UI_meanFleetSplineSpeedMPS)를 램프뿐 아니라 "
            ...
            "들어옵니다."
            "<br><br><b>급커브에서는 플릿만으로 부족합니다.</b> 곡률 구간별로 재보니 헤어핀"
            "(반경 31m)에서 플릿은 28.5mph까지만 내려가는데, 횡가속도 3.0m/s² 기준 곡률한계는 "
            "21.4mph였고 실제로는 17.3mph로 돌았습니다. 플릿 스플라인은 구간 평균이라 가장 조인 "
            "지점이 평탄해집니다."
            ...
            "<br><br><b>실제로 무엇이 상한을 정하는가</b> — engaged 32.2만 프레임에서 플릿 "
            "<b>79.3%</b>, 맵 제한+오프셋 16.2%, 곡률 4.6%였습니다. 플릿이 맵 목표보다 낮은 "
            "경우가 83.8%라 직선에서도 플릿이 잡습니다."
            "<br><br><b>올리지는 않습니다</b> — 플릿 속도는 곡률만이 아니라 교통량·교차로도 "
            "반영하므로 목표를 대체하지 않고 낮추기만 합니다. 감속 자체는 기존 슬루(내려갈 때 "
            "1.0m/s²)를 그대로 따라갑니다.",
    "options": [(0, "사용 안 함"), (1, "사용 (기본)")],
  },
  "TeslaMapCurveLatAccel": {
    "label": "커브 감속 · 쏠림 기준", "type": "int",
    "help": "커브를 돌 때 <b>몸이 옆으로 쏠리는 세기</b>를 어디까지 허용할지입니다. 위 플릿 "
            "속도와 <b>둘 중 낮은 쪽</b>이 상한이 됩니다."
            ...
            "<br><br><b>왜 플릿만으로는 부족한가</b> — 플릿 스플라인은 구간 평균이라 제일 조인 "
            "지점이 뭉개집니다. 반경 31m 헤어핀 실측에서 플릿은 28.5mph까지만 내려갔는데, "
            "3.0 기준 곡률한계는 21.4mph였고 실제로는 <b>17.3mph</b>로 돌았습니다. 반대로 "
            "직선에서는 곡률한계가 100mph쯤이라 아무 구속이 없고 플릿이 일합니다. 서로 못 하는 "
            "쪽을 메웁니다."
            ...
            "<br><br>전방 <b>4초</b>를 봅니다(플릿 신호의 선행 시간과 동일). <b>램프에도 "
            "적용됩니다</b> — 진입 램프의 루프 구간(전체의 14.2%)에서 플릿은 38.6mph로 읽는데 "
            "실제로는 27.8mph로 돌아야 했고, 이 기준은 27.3mph로 맞췄습니다. 합류 직선에서는 "
            "곡률이 없어 한계가 113mph쯤이라 저절로 풀립니다.",
    "options": [(0, "사용 안 함 (플릿만)"), (250, "2.5 일찍 감속"),
...
SETTINGS = {
  ...
  "TeslaCarsAsTrucks": {
    ...
    "help": "테슬라 2026.26.1 업데이트 이후 AP1 계기판이 승용차를 그리지 않습니다. 트럭과 "
            "오토바이는 그대로 그려지므로, 순정이 보낸 물체 목록에서 차종만 트럭으로 바꿔 "
            "다시 보냅니다. 거리·위치·상대속도는 순정 값 그대로입니다. 켜면 앞차가 다시 "
            "보이는 대신 승용차가 트럭 아이콘으로 나옵니다. 재시작해야 반영됩니다.",
    "options": [(0, "사용 안 함 (기본)"), (1, "트럭으로 표시")],
  },
  "TeslaStockAutopark": {
