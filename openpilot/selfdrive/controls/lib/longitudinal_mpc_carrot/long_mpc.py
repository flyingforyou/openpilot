...
if __name__ == '__main__':  # generating code
  ...
  # count. Own directory, own MODEL_NAME, own .so.
  from openpilot.selfdrive.controls.lib.longitudinal_mpc_carrot.c_generated_code.acados_ocp_solver_pyx import AcadosOcpSolverCython

from casadi import SX, vertcat

MODEL_NAME = 'long_carrot'
LONG_MPC_DIR = os.path.dirname(os.path.abspath(__file__))
EXPORT_DIR = os.path.join(LONG_MPC_DIR, "c_generated_code")
JSON_FILE = os.path.join(LONG_MPC_DIR, "acados_ocp_long_carrot.json")
...
def get_stopped_equivalence_factor(v_lead, comfort_brake_2=COMFORT_BRAKE):
  return (v_lead**2) / (2 * comfort_brake_2)
...
def desired_follow_distance(v_ego, v_lead, comfort_brake, stop_distance, t_follow=None, comfort_brake_2=COMFORT_BRAKE):
  if t_follow is None:
    t_follow = get_T_FOLLOW()
  return (get_safe_obstacle_distance(v_ego, t_follow, comfort_brake, stop_distance)
          - get_stopped_equivalence_factor(v_lead, comfort_brake_2))
