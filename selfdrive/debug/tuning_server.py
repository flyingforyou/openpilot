...
SETTINGS = {
  ...
  "TeslaVelocityPid": {
    ...
    "help": "정지 제어를 '가속 명령 그대로 내보내기(피드포워드)'에서 '계획 속도를 닫힌 루프로 추종'으로 "
            "바꿉니다. 지금은 가속 게인이 0이라 정지점을 지나쳐 앞차에 바짝 붙는데, 속도를 직접 추종하면 "
            "계획한 지점에 정확히 섭니다. 게인은 CarrotPilot 기본값(kp 1.0)이라 실차에서 미세 조정이 "
            "필요할 수 있습니다. 변경 후 재시동이 필요합니다.",
