...
SETTINGS = {
  ...
  "RadarLeadHoldCm": {
    "group": "차간 · 추종",
    "label": "근거리 레이더 트랙 유지", "type": "int",
    "help": "리드 선택은 비전 확률(prob>0.5)로 먼저 걸러지기 때문에, 카메라 확신이 잠깐 흔들리면 "
            "앞차를 계속 보고 있는 레이더 트랙까지 같이 버려집니다. 이 로그(365 세그먼트·주행 148분) "
            "에서 앞차가 30m 이내였을 때 레이더→비전 전환 순간 보고 거리가 중앙값 +4.9m, p90 +9.8m "
            "튀었고, 59%가 2m 이상 멀게 읽었습니다(실제 18.4m를 24.8m로). 차가 없는 여유를 있다고 "
            "믿으니 실제 차간이 그만큼 짧아집니다. 이 값 안쪽에서는 비전이 흔들려도 쓰던 레이더 트랙을 "
            "그대로 유지합니다. 이미 비전이 앞차로 확정했던 트랙만 유지하며, 레이더만으로 새 앞차를 "
            "잡지는 않습니다(육교·가드레일 오검출 방지). 0이면 사용 안 함.",
    "options": [(0, "사용 안 함 (기본)"), (2000, "20m 이내"),
                (3000, "30m 이내"), (4000, "40m 이내")],
  },
  "GreyLeadProbPct": {
    "group": "가속 · 감속",
    "label": "앞차 의심되면 가속 멈춤", "type": "int",
    "help": "radard는 모델의 앞차 확률이 0.5를 넘어야 리드를 발행합니다. 그 아래에서는 플래너가 "
            "'앞이 비었다'고 보고 계속 가속합니다. 오늘 로그(0000001b 세그10)에서 정지차를 향해 "
            "확률이 0.01→0.47로 올라가는 5.4초 동안 최대 가속(2.0 m/s²)으로 51→65km/h까지 붙였고, "
            "61m에서야 리드가 발행됐을 땐 이미 필요 감속이 한계(-3.5)를 넘어 운전자가 직접 "
            "밟아야 했습니다. 51km/h를 유지했다면 2.0 m/s²면 충분했습니다. 이 확률 이상이면 "
            "가속만 멈춥니다 — 제동은 그대로 비전 게이트를 따르므로 유령 제동은 생기지 않습니다. "
            "0.2~0.5 구간은 실주행의 2.5%(148분 중 3.7분)에 불과합니다. 0이면 사용 안 함.",
    "options": [(0, "사용 안 함 (기본)"), (20, "확률 0.20부터 (넓게)"),
                (30, "확률 0.30부터"), (40, "확률 0.40부터 (좁게)")],
  },
  "GreyLeadAccelCapCms": {
    "group": "가속 · 감속",
    "label": "가속 멈춤 시 허용 가속도", "type": "int",
    "help": "'앞차 의심되면 가속 멈춤'이 걸린 동안 허용할 최대 가속도입니다. 0이면 속도를 "
            "그대로 유지하고, 조금 남겨두면 오르막에서 속도가 처지는 것을 막아줍니다. "
            "감속에는 전혀 영향이 없습니다.",
    "options": [(0, "0.00 m/s² · 완전 정지 (기본)"), (20, "0.20 m/s² · 오르막 보정"),
                (50, "0.50 m/s² · 약하게만 제한")],
  },
  "RadarLeadHoldMs": {
    "group": "차간 · 추종",
    "label": "레이더 트랙 유지 시간", "type": "int",
    "help": "비전이 흔들리는 동안 레이더 트랙을 최대 얼마나 붙들지입니다. 이 시간을 넘기면 놓아줍니다. "
            "레이더가 트랙을 놓거나 트랙이 튀면 시간과 무관하게 즉시 해제되고, 비전이 다시 같은 차를 "
            "잡으면 예산이 새로 채워집니다. 로그상 리드 소실은 중앙값 0.10초·p90 1.0초라 1초면 대부분을 "
            "덮습니다. '근거리 레이더 트랙 유지'가 켜져 있을 때만 동작합니다.",
    "options": [(500, "0.5초"), (1000, "1.0초 (기본)"), (1500, "1.5초"), (2500, "2.5초")],
  },
  # ── 가속 · 감속 ────────────────────────────────────────────────────────────
  "LaunchAccelCms": {
    "group": "가속 · 감속",
